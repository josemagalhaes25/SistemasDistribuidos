...
namespace WavyApp
{
    internal class Program
    {
        ...
        static List<string> sensores = new() {  "VelocidadeCorrenteza", "DensidadePlankton", "TemperaturaFundo", "NivelRuidoSubaquatico" };
        ...
        static void GenerateData()
        {
            while (running)
            {
                foreach (var sensor in sensores)
                {
                    ...
                    {
                        "VelocidadeCorrenteza" => (0 + rand.NextDouble() * 2).ToString("F2"),         // m/s
                        "DensidadePlankton" => (1000 + rand.NextDouble() * 9000).ToString("F0"),     // células/mL
                        "TemperaturaFundo" => (4 + rand.NextDouble() * 6).ToString("F1"),            // ºC
                        "NivelRuidoSubaquatico" => (60 + rand.NextDouble() * 40).ToString("F0"),     // dB
                        _ => "0"
                    };
