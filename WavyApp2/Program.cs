...
namespace WavyApp
{
    internal class Program
    {
        ...
        static List<string> sensores = new() { "Turbidez", "PH", "Oxigenio", "Corrente" };
        ...
        static void GenerateData()
        {
            while (running)
            {
                foreach (var sensor in sensores)
                {
                    ...
                    {
                        "Turbidez" => (0 + rand.NextDouble() * 50).ToString("F1"),
                        "PH" => (6 + rand.NextDouble() * 2).ToString("F2"),
                        "Oxigenio" => (4 + rand.NextDouble() * 5).ToString("F1"),
                        "Corrente" => (0 + rand.NextDouble() * 3).ToString("F1"),
                        _ => "0"
                    };
