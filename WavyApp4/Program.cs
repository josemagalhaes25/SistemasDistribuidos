...
namespace WavyApp
{
    internal class Program
    {
        ...
        static List<string> sensores = new() { "CO2Dissolvido", "CondutividadeEletrica", "PressaoAtmosferica", "AlturaMare" };
        ...
        static void GenerateData()
        {
            while (running)
            {
                foreach (var sensor in sensores)
                {
                    ...
                    {
                        "CO2Dissolvido" => (2 + rand.NextDouble() * 3).ToString("F2"),             // mg/L
                        "CondutividadeEletrica" => (100 + rand.NextDouble() * 900).ToString("F1"),// µS/cm
                        "PressaoAtmosferica" => (980 + rand.NextDouble() * 40).ToString("F0"),    // hPa
                        "AlturaMare" => (0 + rand.NextDouble() * 3).ToString("F2"),               // metros
                        _ => "0"
                    };
