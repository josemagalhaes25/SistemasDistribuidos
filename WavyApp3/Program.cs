...
namespace WavyApp
{
    internal class Program
    {
        ...
        static List<string> sensores = new() { "Nitratos", "Clorofila", "RadiacaoSolar", "TransparenciaAgua" };
        ...
        static void GenerateData()
        {
            while (running)
            {
                foreach (var sensor in sensores)
                {
                    ...
                    {
                        "Nitratos" => (0 + rand.NextDouble() * 10).ToString("F2"),              // mg/L
                        "Clorofila" => (0 + rand.NextDouble() * 50).ToString("F1"),             // µg/L
                        "RadiacaoSolar" => (200 + rand.NextDouble() * 800).ToString("F0"),     // W/m²
                        "TransparenciaAgua" => (0.5 + rand.NextDouble() * 5).ToString("F2"),   // metros
                        _ => "0"
                    };
